...
Every figure here comes from a command in this repo, run on the machine
named beside it. Ratios and byte counts reproduce anywhere; frame times
are quoted with their spread, because they move with whatever else the
machine is doing.

The short version lives in the [README](../README.md#the-numbers). This
is the long one: the sweeps, the pass breakdowns, the things that turned
out not to be true, and the reasoning behind each gate.
...
| Mesher differential fuzz (`ctest -R mesher_equivalence`) | every quad from both meshers decomposed back into 1x1 unit faces and compared as sets: same cells, same facing, same block id, no duplicates. 10,020 cases (12 fills x 5 neighbour configurations x 167 seeds). Seeds are the only axis this actually searches - the fills and neighbour configurations are fixed lists, and a randomized fill draws a different chunk every seed - so that is the axis that was widened, from 3. The sanitizer build runs the same binary at MESHER_FUZZ_SEEDS=12, since what it looks for is a memory error on the mesher's indexing, which every case exercises. Reintroducing the boundary-ownership defect fails 99 of its checks while the world-unit suite beside it passes regardless, which is why the fuzzer is a separate binary. The check count is deliberately not quoted here: it moves every time a test is added, and a number that rots on unrelated work is the kind this README keeps having to correct |
...
![Emissive blocks lighting the terrain at night](media/block_light.jpg)
...
| greedy | naive |
| :---: | :---: |
| ![One chunk meshed greedily: flat spans are single large rectangles](media/mesher_greedy.jpg) | ![The same chunk meshed naively: one quad per visible block face](media/mesher_naive.jpg) |
