...
`tests/test_noise4d.cpp`, 36 checks, run by ctest. The properties, not the
outputs:
...
on an Apple M4:
    ...
    full rebuild per w step: 193 ms  (gen 93 + mesh 100)
...
| --- | --- | --- |
| chunk (today's mesher) | **100%** | 193 ms, 1.00x |
| section (renderer's unit) | **19-23%** | 113 ms, **1.70x** |
| column (theoretical floor) | 58-65% | 155 ms, 1.24x |
...
rather than a guess: 193 ms single-threaded per step at radius 6, about
21 ms across nine workers, and 13 ms if meshing moved to section
granularity.
