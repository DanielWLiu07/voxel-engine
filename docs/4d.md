...
Nothing in the engine is 4D. This is a noise field, a harness, and an
answer. The engine still stores 3D chunks, meshes them in 3D, and renders
them in 3D, and every existing number is unchanged: `--validate` still
reports `gpu_mesh_mb=10.99`, and `check_invariance.py` still passes.
...
The open engineering problem is `w` traversal. A step along `w` invalidates
every mesh, and at today's 5.04 ms per chunk that is roughly 350 ms across
nine workers - a visible hitch every time the player moves in the fourth
dimension. Three approaches worth measuring, in order: extending greedy
meshing to merge along `w` so re-slicing becomes a query rather than a
rebuild; an LRU of slice meshes, trading GPU memory for smoothness; and
incremental re-meshing of only the chunks whose contents actually differ
between adjacent slices.

That cost is undocumented as far as I can find. Games that do this do not
publish it.
