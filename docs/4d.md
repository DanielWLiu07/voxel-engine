...
on an Apple M4:
    ...
    full rebuild per w step: 188 ms  (gen 91 + mesh 97)
...
| --- | --- | --- |
| chunk (today's mesher) | **100%** | 188 ms, 1.00x |
| section (renderer's unit) | **~22%** | 111 ms, **1.70x** |
| column (theoretical floor) | ~56% | 147 ms, 1.28x |
...
rather than a guess: 188 ms single-threaded per step at radius 6, about
21 ms across nine workers, and 12 ms if meshing moved to section
granularity.
