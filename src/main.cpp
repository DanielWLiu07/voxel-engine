...
int main(int argc, char** argv) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (validate_mode && world_settled) {
            // Not the render loop's job: see bench/validate_mode.h.
            switch (bench::run_validate(wrld, terrain, pool)) {
            case bench::ModeResult::Failed:   return EXIT_FAILURE;
            case bench::ModeResult::Finished: break;
            }
            glfwSetWindowShouldClose(window, GLFW_TRUE);
        }
        ...
        if (bench_edit > 0 && world_settled) {
            // Not the render loop's job: see bench/edit_bench.h.
            switch (bench::run_edit_bench(wrld, bench_edit, stream_radius)) {
            case bench::ModeResult::Failed:   return EXIT_FAILURE;
            case bench::ModeResult::Finished: break;
            }
            glfwSetWindowShouldClose(window, GLFW_TRUE);
