...
void PostProcess::destroy() {
    ...
    if (scene_color_) { glDeleteTextures(1, &scene_color_);     scene_color_ = 0; }
    if (scene_depth_) { glDeleteRenderbuffers(1, &scene_depth_); scene_depth_ = 0; }
    for (int i = 0; i < 2; ++i) {
...
bool PostProcess::init(int w, int h) {
    ...
    // Scene HDR FBO: RGBA16F color + depth renderbuffer (we never sample
    // depth from this pipeline, so a renderbuffer is cheaper than a texture).
    scene_color_ = make_color_texture(w, h, GL_RGBA16F);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                           GL_TEXTURE_2D, scene_color_, 0);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                              GL_RENDERBUFFER, scene_depth_);
    GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        std::fprintf(stderr, "[postfx] scene FBO incomplete: 0x%X\n", status);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            std::fprintf(stderr, "[postfx] bloom FBO %d incomplete: 0x%X\n", i, status);
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
            destroy();
            return false;
        }
...
void PostProcess::resolve_to_backbuffer(const Shader& bright_extract,
    ...
    // Bright extract: scene -> bloom_fbo[0]
    ...
    // Separable Gaussian: alternate horizontal/vertical, ping-pong between
    // bloom_fbo_[0] and bloom_fbo_[1]. Output of bright extract is currently
    // in [0]; first blur reads [0] -> [1].
    blur.use();
