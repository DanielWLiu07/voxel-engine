...
bool ShadowMap::init(int size) {
    ...
    // Enable hardware PCF — sampler2DShadow returns a 0..1 comparison
    // result; the GPU's bilinear filter blends 4 nearest depth tests.
    ...
    // Depth-only: explicitly disable color attachments.
...
glm::mat4 ShadowMap::fit_view_proj(const glm::vec3& center,
    ...
    // Camera placed `depth_extent` units back from the center along the
    // light direction. Looking toward the center. Up = world Y (or Z
    // when the sun is overhead, to avoid degenerate cross product).
    glm::vec3 up = (std::abs(L.y) > 0.95f)
